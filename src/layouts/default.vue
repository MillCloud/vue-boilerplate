<template>
  <el-container class="relative min-h-screen bg-gray-4">
    <el-aside
      class="fixed z-20 flex flex-col w-64 h-screen transition bg-white border-0 border-r border-solid border-gray-5"
      :style="{
    ...
    <el-container class="relative flex-col min-h-screen" :class="{ 'is-wider': isAsideCollapsed }">
      <el-header
        class="fixed z-10 flex-none w-full transition bg-white border-0 border-b border-solid border-gray-5"
      >
...
const defaultActive = computed(() => {
  const menuItem = menuItems.value.find(
    (item) => item.path.includes(route.value.path) || route.value.path.includes(item.path),
  );
  if (!menuItem) {
    return '';
  }
  ...
  const menuItemChild = menuItem.children.find((child) => child.path === route.value.path);
